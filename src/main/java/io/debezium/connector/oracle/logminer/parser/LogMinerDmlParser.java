...
public class LogMinerDmlParser implements DmlParser {
    ...
    private LogMinerDmlEntry parseInsert(String sql, Table table) {
        try {
            ...
            List<String> columnNames = new ArrayList<>(table.columns().size() + 10);
     ...
     */
    private int parseColumnValuesClause(String sql, int start, List<String> columnNames, Object[] values, Table table) {
        int index = start;
        int nested = 0;
        boolean inQuote = false;
        ...
        for (; index < sql.length(); ++index) {
            ...
            if (c == '(' && !inQuote && !inValues) {
                ...
                if (sql.charAt(start) == '\'' && sql.charAt(index - 1) == '\'') {
                    ...
                    int position = LogMinerHelper.getColumnIndexByName(columnNames.get(columnIndex), table);
                    if (position >= 0) {
